...
class MainHandler(BaseHandler):
    def get(self):
        ...
        if user is None:
            return self.redirect("/splash")
        # grabbing five most latest issues from ALL repositories
        access_url = "https://api.github.com/user/issues?access_token=" + self.session.get(
            "access_token") + "&filter=all&direction=dec"
        ...
        access_url = "https://api.github.com/user/repos?access_token=" + self.session.get(
            "access_token") + "&filter=all&sort=pushed&direction=dec"
        ...
        for repo in five_last_updated_repo_times:
            repo_date = datetime.strptime(five_last_updated_repo_times[repo_count].get('pushed_at')[0:18],
                                          "%Y-%m-%dT%H:%M:%S")
            repo_count += 1
            if len(five_most_recent_commits) == 5:
                break
            full_name = repo.get('full_name')
            access_url = "https://api.github.com/repos/" + full_name + "/commits?access_token=" + self.session.get(
                "access_token")
        ...
        print BaseHandler.user(self)
        self.response.write(template.render(name="Dashboard", issues=issueList, commits=five_most_recent_commits,
                                            token=self.session.get("access_token"), user=BaseHandler.user(self)))


class Splash(BaseHandler):
    def get(self):
        template = env.get_template('splash.html')
        self.response.write(template.render(name="Welcome"))
...
class Register(BaseHandler):
    ...
    def post(self):
        ...
        person = User(user_id=self.session.get("username"), first_name=first, last_name=last, email=email,
                      gravatar_url=self.session.get("gravatar"))
        person.put()
...
class ProjectDashboard(BaseHandler):
    def get(self):
        ...
        self.response.write(
            template.render(name="Project Overview", project=project_contents, user=BaseHandler.user(self)))
    ...
    def post(self):
        ...
        project = Project(project_id=int(project_id), project_title=project_name, project_desc=project_description,
                          project_owner=owner)
        project.put()
